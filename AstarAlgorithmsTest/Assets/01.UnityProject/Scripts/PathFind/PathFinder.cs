...
public class PathFinder : GSingleton<PathFinder>
{
    ...
    {
        // Astar 알고리즘을 사용하기 위해서 패스 리스트를 초기화 한다.
        ...
        while(isFoundDestination == false && isNowayToGo == false)
        {
            ...
            {
                // { 도착하지 않았다면 현재 타을을 기준으로 4 방향 노드를 찾아온다.
                ...
                // 탐색이 끝난 노드는 Clost list 에 추가하고, Open list 에서 제거한다.
                ...
                {
                    GFunc.LogWarning("[Warning] Thre are no more tiles to explore.");
                    isNowayToGo = true;
                }       // if :  목적지에 도착하지 못했는데, 더 이상 탐색할 수 있는 길이 없는경우
        ...
        AstarNode targetTerrain_, AstarNode prevNode = default)
    {
        ...
        {
            // 여기는 이미 탐색이 끝난 좌표의 노드가 존재하는 경우에는
        ...
        else
        {
            ...
            {
                aStarOpenPath.Add(targetTerrain_);
            }       //else : Open List 에 현재 추가할 노드와 같은 좌표의 노드가 없는경우
    ...
    //! Target 지형의 정보와 Destination 지형 정보로 Distance 와 Heuristic 을 설정하는 함수
